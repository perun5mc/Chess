...
void selectLastMove(vector<pair<string, string>> list, bool animate = true)
{
    animating = animate;
...
void ChangeMoveSelection(int by)
{
    ...
    if (chessGame->GetMinSelectedMove0() && selectedMoveIndex + by < 0)
    {
        return;
    }
...
bool MovePiece(string from, string to, string promotion, bool animate = true)
{
    ...
    {
        scrollToBottom();
        selectLastMove(chessGame->GetMoveList(), animate);
    }
            ...
            mousePos.y < boardOffsetY + boardHeight)
        {
            ...
            else
            {
                if (pieceBelongsToCurrentPlayer)
                {
                    ...
                    {
                        if (!MovePiece(selectedSquare, newSquare, "", !isDragging))
                        {
                            if (!isDragging)
                            {
                                selectedSquare = newSquare;
...
int prevMoveIndexWatcher = -2;
int prevMoveIndex = -1;

void drawChessBoard()
{
    if (prevMoveIndexWatcher != selectedMoveIndex)
    {
        if (prevMoveIndexWatcher != -2)
        {
            prevMoveIndex = prevMoveIndexWatcher;
        }
        prevMoveIndexWatcher = selectedMoveIndex;
    }
    ...
    auto getVisualPosition = [&](const std::string &square) -> Vector2
    {
        if (square == "-")
            return {-1000, -1000};
        ...
        float x = boardOffsetX + visualCol * squareSize + squareSize / 2;
        float y = boardOffsetY + visualRow * squareSize + squareSize / 2;
    ...
    if (prevMoveIndex == selectedMoveIndex && animating)
    {
        animating = false;
        currentAnimationProgress = 0;
    }
    ...
    auto getTextureKeyFromId = [](const std::string &pieceId) -> std::string
    {
        if (pieceId.find("white_") == 0)
        {
            if (pieceId.find("pawn") != std::string::npos)
                return "P";
            if (pieceId.find("knight") != std::string::npos)
                return "N";
            if (pieceId.find("bishop") != std::string::npos)
                return "B";
            if (pieceId.find("rook") != std::string::npos)
                return "R";
            if (pieceId.find("queen") != std::string::npos)
                return "Q";
            if (pieceId.find("king") != std::string::npos)
                return "K";
        }
        else if (pieceId.find("black_") == 0)
        {
            if (pieceId.find("pawn") != std::string::npos)
                return "p";
            if (pieceId.find("knight") != std::string::npos)
    ...
    if (animating)
    {
        unordered_map<string, string> prevMap = chessGame->GetPieceMapAtIndex(prevMoveIndex + 1);
        unordered_map<string, string> newMap = chessGame->GetPieceMapAtIndex(selectedMoveIndex + 1);

        prevMap = fillSpawnedPieces(prevMap, newMap);

        vector<pair<string, string>> stationaryPieces;
        vector<pair<string, string>> movingPieces;
        ...
        for (const auto &[pieceId, newPos] : newMap)
        {
            ...
            auto it = prevMap.find(pieceId);
            std::string prevPos = (it != prevMap.end()) ? it->second : "-";
            ...
            if (prevPos == newPos)
            {
                stationaryPieces.push_back({pieceId, newPos});
            }
            else
            {
                movingPieces.push_back({pieceId, newPos});
            }
        }

        for (const auto &[pieceId, newPos] : stationaryPieces)
        {
            auto it = prevMap.find(pieceId);
            std::string prevPos = (it != prevMap.end()) ? it->second : "-";
            ...
            Vector2 pos = getVisualPosition(newPos);

            std::string textureKey = getTextureKeyFromId(pieceId);
            if (textureKey.empty())
                continue;

            auto textureIt = pieceTextures.find(textureKey);
            if (textureIt == pieceTextures.end())
                continue;

            const Texture2D &texture = textureIt->second;

            float scale = (float)squareSize / std::max(texture.width, texture.height) * textureScale;
            int width = texture.width * scale;
            int height = texture.height * scale;
        ...
        for (const auto &[pieceId, newPos] : movingPieces)
        {
            auto it = prevMap.find(pieceId);
            std::string prevPos = (it != prevMap.end()) ? it->second : "-";
            ...
            Vector2 fromPos = getVisualPosition(prevPos);
            Vector2 toPos = getVisualPosition(newPos);
            ...
            Vector2 currentPos = Vector2Lerp(fromPos, toPos, EaseInOutCubic(currentAnimationProgress));

            currentPos.x -= width / 2.0f;
            currentPos.y -= height / 2.0f;

            DrawTextureEx(texture, currentPos, 0.0f, scale, WHITE);
        }
...
void drawSidePanel()
{
    ...
    if (GuiButton(undoButton, "Undo move"))
    {
        if (!showBoardsDropdown && !showPieceSetDropdown)
        {
            ...
            {
                scrollToBottom();
                selectLastMove(chessGame->GetMoveList());
            };
        }
    }
